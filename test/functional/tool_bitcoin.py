...
"""Test the truenorth wrapper tool."""
...
class ToolBitcoinTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `truenorth node -version` is
        # run on windows, python doesn't capture output from the child
        # `truenorthd` and `truenorth-node` process started with _wexecvp, and
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through truenorth wrapper command with specified arguments."""
    ...
    def run_test(self):
        ...
        self.log.info("Ensure truenorth node command invokes truenorthd by default")
        self.test_args([], [], expect_exe="truenorthd")

        self.log.info("Ensure truenorth -M invokes truenorthd")
        self.test_args(["-M"], [], expect_exe="truenorthd")

        self.log.info("Ensure truenorth -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure truenorth -m invokes truenorth-node")
            self.test_args(["-m"], [], expect_exe="truenorth-node")

            self.log.info("Ensure truenorth -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="truenorth-node")

            self.log.info("Ensure truenorth accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="truenorth-node")

            self.log.info("Ensure truenorth recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="truenorth-node")
