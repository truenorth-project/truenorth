...
class WalletSendTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        assert_raises_rpc_error(-8, "Use fee_rate (mie/vB) instead of feeRate", w0.send, {w1.getnewaddress(): 1}, 6, "conservative", 1, {"feeRate": 0.01})
        ...
        # Test setting explicit fee rate just below the minimum of 1 mie/vB.
        self.log.info("Explicit fee rate raises RPC error 'fee rate too low' if fee_rate of 0.99999999 is passed")
        msg = "Fee rate (0.999 mie/vB) is lower than the minimum fee rate setting (1.000 mie/vB)"
        ...
        msg = "Fee rate (0.000 mie/vB) is lower than the minimum fee rate setting (1.000 mie/vB)"
        ...
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=SUBSIDY + 1, inputs=[], add_to_wallet=False)
        assert res["complete"]
        utxo1 = w0.listunspent()[0]
        assert_equal(utxo1["amount"], SUBSIDY)
        ERR_NOT_ENOUGH_PRESET_INPUTS = "The preselected coins total amount does not cover the transaction target. " \
                                       "Please allow other inputs to be automatically selected or include more coins manually"
        self.test_send(from_wallet=w0, to_wallet=w1, amount=SUBSIDY + 1, inputs=[utxo1],
                       expect_error=(-4, ERR_NOT_ENOUGH_PRESET_INPUTS))
        self.test_send(from_wallet=w0, to_wallet=w1, amount=SUBSIDY + 1, inputs=[utxo1], add_inputs=False,
                       expect_error=(-4, ERR_NOT_ENOUGH_PRESET_INPUTS))
        res = self.test_send(from_wallet=w0, to_wallet=w1, amount=SUBSIDY + 1, inputs=[utxo1], add_inputs=True, add_to_wallet=False)
        assert res["complete"]
        ...
        # Fund ext_wallet with P2WPKH, not the P2QRH default: a P2QRH input is
        # signed with a BIP341-style sighash that commits to every input's
        # prevout, and ext_wallet has no UTXO data for the external input, so
        # it could not sign its own input in the PSBT below.
        self.nodes[0].sendtoaddress(ext_wallet.getnewaddress(address_type="bech32"), 10)
