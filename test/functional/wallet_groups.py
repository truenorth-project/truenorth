...
class WalletGroupTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.extra_args = [
            ...
            [],
            ["-avoidpartialspends"],
            # -maxapsfee thresholds and fee constants below assume all-P2WPKH
            # transactions, so pin change to bech32 (default change is P2QRH).
            ["-maxapsfee=0.00002719", "-changetype=bech32"],
            ["-maxapsfee=0.00002720", "-changetype=bech32"],
        ]
    ...
    def run_test(self):
        ...
        # The expected fees and the -maxapsfee thresholds are calibrated for
        # P2WPKH inputs and outputs, so request bech32 instead of the P2QRH default.
        ...
        with self.nodes[3].assert_debug_log([f'Fee non-grouped = {tx5_6_ungrouped_fee}, grouped = {tx5_6_grouped_fee}, using non-grouped']):
            txid5 = self.nodes[3].sendtoaddress(self.nodes[0].getnewaddress(address_type="bech32"), 2.95)
        ...
        with self.nodes[4].assert_debug_log([f'Fee non-grouped = {tx5_6_ungrouped_fee}, grouped = {tx5_6_grouped_fee}, using grouped']):
            txid6 = self.nodes[4].sendtoaddress(self.nodes[0].getnewaddress(address_type="bech32"), 2.95)
