...
# The regtest block reward (SUBSIDY below the first halving at height 150)
# matures after COINBASE_MATURITY (100) blocks. Therefore, after mining 101
# blocks we expect node 0 to have a balance of
# (BLOCKS - COINBASE_MATURITY) * SUBSIDY.
...
WALLET_NOT_SPECIFIED = (
    ...
    "option before the command (run \"truenorth-cli -h\" for help or \"truenorth-cli listwallets\" to see "
...
class TestBitcoinCli(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        if self.is_wallet_compiled():
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (NORTH/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (NORTH/kvB)']), network_info['relayfee'])
            assert_equal(self.nodes[0].cli.getwalletinfo(), wallet_info)

            # Setup to test -getinfo, -generate, and -rpcwallet= with multiple wallets.
            wallets = [self.default_wallet_name, 'Encrypted', 'secret']
            amounts = [BALANCE + SUBSIDY - Decimal('40.000072'), Decimal(9), Decimal(31)]
            ...
            # Mine a block to confirm; adds a block reward (SUBSIDY) to the default wallet.
