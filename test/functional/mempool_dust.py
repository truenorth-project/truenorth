...
DUST_RELAY_TX_FEE = 3000  # default setting [mie/kvB]
...
class DustRelayFeeTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        # test default (no parameter), disabled (=0) and a bunch of arbitrary dust fee rates [mie/kvB]
        for dustfee_sat_kvb in (DUST_RELAY_TX_FEE, 0, 1, 66, 500, 1337, 12345, 21212, 333333):
            ...
            if dustfee_sat_kvb == DUST_RELAY_TX_FEE:
                self.log.info(f"Test default dust limit setting ({dustfee_sat_kvb} mie/kvB)...")
            else:
                dust_parameter = f"-dustrelayfee={dustfee_btc_kvb:.8f}"
                self.log.info(f"Test dust limit setting {dust_parameter} ({dustfee_sat_kvb} mie/kvB)...")
