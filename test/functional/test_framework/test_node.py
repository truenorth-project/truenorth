...
BITCOIN_PID_FILENAME_DEFAULT = "truenorthd.pid"
...
class TestNode():
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, binaries, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, v2transport=False, uses_wallet=False, ipcbind=False):
        ...
        self.bitcoinconf = self.datadir_path / "truenorth.conf"
        ...
        # Configuration for logging is set as command-line args rather than in the truenorth.conf file.
    ...
    # Deterministic per-node keys. Base58 encodings are chain specific -- the
    # PUBKEY_ADDRESS and SECRET_KEY prefixes differ between regtest (111/239)
    # and signet (127/255) -- so the same underlying keys are stored once per
    # chain that needs them. A single regtest-encoded list made every signet
    # test fail at importprivkey.
    PRIV_KEYS_BY_CHAIN = {
        'regtest': [
            # address , privkey
    ...
    # Most callers are regtest-only and reference this directly.
    PRIV_KEYS = PRIV_KEYS_BY_CHAIN['regtest']

    def get_deterministic_priv_key(self):
        """Return a deterministic priv key in base58, that only depends on the node's index"""
        keys = self.PRIV_KEYS_BY_CHAIN.get(self.chain, self.PRIV_KEYS_BY_CHAIN['regtest'])
        assert len(keys) == MAX_NODES
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # truenorth.conf. To avoid collisions, change it to 127.0.0.1:tor_port().
