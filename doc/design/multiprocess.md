...
This subdivision could be extended in the future. For example, indexes could be removed from the `truenorth-node` executable and run in separate executables. And JSON-RPC servers could be added to wallet and index executables, so they can listen and respond to RPC requests on their own ports, without needing to forward RPC requests through `truenorth-node`.

<table><tr><td>

```mermaid
flowchart LR
    node[truenorth-node] -- listens on --> socket["&lt;datadir&gt;/node.sock"]
    wallet[truenorth-wallet] -- connects to --> socket
...
Let’s walk through an example where the `truenorth-wallet` process requests the hash of a block at a specific height from the `truenorth-node` process. This example demonstrates the practical application of the IPC mechanism, specifically the interplay between C++ method calls and Cap’n Proto-generated RPC calls.

<table><tr><td>

```mermaid
sequenceDiagram
    box "truenorth-wallet process"
...
1. **Initiation in truenorth-wallet**
...
3. **Request Preparation and Dispatch**
   - The `getBlockHash` method of the generated `Chain` client subclass in `truenorth-wallet` populates a Cap’n Proto request with the `height` parameter, sends it to `truenorth-node` process, and waits for a response.

4. **Handling in truenorth-node**
   - Upon receiving the request, the Cap'n Proto dispatching code in the `truenorth-node` process calls the `getBlockHash` method of the `Chain` [server class](#c-server-classes-in-generated-code).
   ...
   - The `getBlockHash` method of the generated `Chain` server subclass in `truenorth-wallet` receives a Cap’n Proto request object with the `height` parameter, and calls the `getBlockHash` method on its local `Chain` object with the provided `height`.
   - When the call returns, it encapsulates the return value in a Cap’n Proto response, which it sends back to the `truenorth-wallet` process.

5. **Response and Return**
   - The `getBlockHash` method of the generated `Chain` client subclass in `truenorth-wallet` which sent the request now receives the response.
...
- Separating indexes from `truenorth-node`, and running indexing code in separate processes (see [indexes: Stop using node internal types #24230](https://github.com/bitcoin/bitcoin/pull/24230)).
