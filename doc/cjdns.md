...
CJDNS peers than Tor or IP ones. You can use `truenorth-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/truenorth-project/truenorth/blob/master/doc/tor.md) for
details.
...
To see which CJDNS peers your node is connected to, use `truenorth-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `truenorth-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `truenorth-cli help getnodeaddresses` for details.
