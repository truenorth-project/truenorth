...
Although there is already a brief explanation about the multisig in the [Descriptors documentation](https://github.com/truenorth-project/truenorth/blob/master/doc/descriptors.md#multisig), this tutorial proposes to use the signet (instead of regtest), bringing the reader closer to a real environment and explaining some functions in more detail.
...
```bash
./build/bin/truenorthd -signet -daemon
```
...
for ((n=1;n<=3;n++))
do
 ./build/bin/truenorth-cli -signet createwallet "participant_${n}"
done
```
...
for ((n=1;n<=3;n++))
do
 xpubs["internal_xpub_${n}"]=$(./build/bin/truenorth-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/1/*"))][0] | .desc' | grep -Po '(?<=\().*(?=\))')

 xpubs["external_xpub_${n}"]=$(./build/bin/truenorth-cli -signet -rpcwallet="participant_${n}" listdescriptors | jq '.descriptors | [.[] | select(.desc | startswith("wpkh") and contains("/0/*") )][0] | .desc' | grep -Po '(?<=\().*(?=\))')
done
```
...
external_desc_sum=$(./build/bin/truenorth-cli -signet getdescriptorinfo $external_desc | jq '.descriptor')
internal_desc_sum=$(./build/bin/truenorth-cli -signet getdescriptorinfo $internal_desc | jq '.descriptor')
...
Documentation for these and other parameters can be found by typing `./build/bin/truenorth-cli help importdescriptors`.
...
```bash
./build/bin/truenorth-cli -signet -named createwallet wallet_name="multisig_wallet_01" disable_private_keys=true blank=true

./build/bin/truenorth-cli  -signet -rpcwallet="multisig_wallet_01" importdescriptors "$multisig_desc"

./build/bin/truenorth-cli  -signet -rpcwallet="multisig_wallet_01" getwalletinfo
```
...
```bash
for ((n=1;n<=3;n++)); do ./build/bin/truenorth-cli -signet loadwallet "participant_${n}"; done
```
...
```bash
receiving_address=$(./build/bin/truenorth-cli -signet -rpcwallet="multisig_wallet_01" getnewaddress)

./contrib/signet/getcoins.py -c ./build/bin/truenorth-cli -a $receiving_address
```
...
```bash
./build/bin/truenorth-cli -signet -rpcwallet="multisig_wallet_01" getbalances
```
...
```bash
balance=$(./build/bin/truenorth-cli -signet -rpcwallet="multisig_wallet_01" getbalance)

amount=$(echo "$balance * 0.8" | bc -l | sed -e 's/^\./0./' -e 's/^-\./-0./')

destination_addr=$(./build/bin/truenorth-cli -signet -rpcwallet="participant_1" getnewaddress)

funded_psbt=$(./build/bin/truenorth-cli -signet -named -rpcwallet="multisig_wallet_01" walletcreatefundedpsbt outputs="{\"$destination_addr\": $amount}" | jq -r '.psbt')
```
...
```bash
./build/bin/truenorth-cli -signet decodepsbt $funded_psbt

./build/bin/truenorth-cli -signet analyzepsbt $funded_psbt
```
...
```bash
psbt_1=$(./build/bin/truenorth-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq '.psbt')

psbt_2=$(./build/bin/truenorth-cli -signet -rpcwallet="participant_2" walletprocesspsbt $funded_psbt | jq '.psbt')
```
...
```bash
combined_psbt=$(./build/bin/truenorth-cli -signet combinepsbt "[$psbt_1, $psbt_2]")
```
...
```bash
finalized_psbt_hex=$(./build/bin/truenorth-cli -signet finalizepsbt $combined_psbt | jq -r '.hex')

./build/bin/truenorth-cli -signet sendrawtransaction $finalized_psbt_hex
```
...
```bash
psbt_1=$(./build/bin/truenorth-cli -signet -rpcwallet="participant_1" walletprocesspsbt $funded_psbt | jq -r '.psbt')

psbt_2=$(./build/bin/truenorth-cli -signet -rpcwallet="participant_2" walletprocesspsbt $psbt_1 | jq -r '.psbt')

finalized_psbt_hex=$(./build/bin/truenorth-cli -signet finalizepsbt $psbt_2 | jq -r '.hex')
