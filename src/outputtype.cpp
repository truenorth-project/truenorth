...
static const std::string OUTPUT_TYPE_STRING_BECH32M_QRH = "bech32m-qrh";
...
std::optional<OutputType> ParseOutputType(const std::string& type)
{
    ...
    } else if (type == OUTPUT_TYPE_STRING_BECH32M_QRH) {
...
const std::string& FormatOutputType(OutputType type)
{
    ...
    case OutputType::BECH32M_QRH: return OUTPUT_TYPE_STRING_BECH32M_QRH;
...
CTxDestination AddAndGetDestinationForScript(FlatSigningProvider& keystore, const CScript& script, OutputType type)
{
    ...
    case OutputType::UNKNOWN: {} // This function should not be used for BECH32M, BECH32M_QRH, or UNKNOWN, so let it assert
    } // no default case, so the compiler can warn about missing cases
    assert(false);
}
...
std::optional<OutputType> OutputTypeFromDestination(const CTxDestination& dest) {
    ...
    if (std::holds_alternative<WitnessV2QRH>(dest)) {
        return OutputType::BECH32M_QRH;
    }
    return std::nullopt;
}
