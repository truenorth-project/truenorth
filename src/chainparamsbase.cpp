...
{
    // RPC ports per chain. P2P port - 1, same pairing as upstream. These
    // don't collide with Bitcoin's ports, so a misconfigured CLI gets
    // connection-refused rather than accidentally talking to a Bitcoin node.
    switch (chain) {
    case ChainType::MAIN:
        return std::make_unique<CBaseChainParams>("", 9554);
    case ChainType::TESTNET:
        return std::make_unique<CBaseChainParams>("testnet3", 19554);
    case ChainType::TESTNET4:
        return std::make_unique<CBaseChainParams>("testnet4", 49554);
    case ChainType::SIGNET:
        // signet keeps Bitcoin's P2P port (38333), so RPC stays at 38332.
        return std::make_unique<CBaseChainParams>("signet", 38332);
    case ChainType::REGTEST:
        // regtest is local-only, Bitcoin's regtest port is fine to reuse.
