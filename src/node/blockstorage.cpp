    ...
    while (pcursor->Valid()) {
        ...
        if (pcursor->GetKey(key) && key.first == DB_BLOCK_INDEX) {
            ...
            if (pcursor->GetValue(diskindex)) {
                ...
                // No PoW recheck on index load: under RandomX (RandomNorth)
                // each hash is ~1ms plus a ~1-2s seed-cache reinit at every
                // 2048-block epoch boundary, so on a chain of any meaningful
                // length this would dominate startup time. The block index
                // entry was only written after full validation accepted the
                // block; LevelDB per-entry checksums and the chainwork
                // comparator handle on-disk-corruption defenses. Bitcoin
                // Core upstream does check PoW here, essentially for free
                // under SHA256d -- the trade-off inverts under RandomX.
...
bool BlockManager::ReadBlock(CBlock& block, const FlatFilePos& pos, const std::optional<uint256>& expected_hash) const
{
    ...
    // No PoW recheck here: every production caller subsequently runs
    // CheckBlock(), which checks PoW via CheckBlockHeader() (gated on
    // fCheckPOW). Doing it twice was effectively free under SHA256d but
    // costs ~1ms per call under RandomX. The expected_hash check below
    // catches on-disk corruption regardless -- any byte change in the
    // deserialized block bytes alters its SHA256d and trips that check.
