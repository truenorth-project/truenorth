...
FUZZ_TARGET(block, .init = initialize_block)
{
    ...
    // Fuzzed blocks have no chain context; use kGenesisSeed for the PoW
    // path. We're exercising CheckBlock's serialisation + structural checks,
    // not consensus correctness against a real chain tip.
    const uint256 seed_key{uint256::ZERO};
    BlockValidationState validation_state_pow_and_merkle;
    const bool valid_incl_pow_and_merkle = CheckBlock(block, validation_state_pow_and_merkle, consensus_params, seed_key, /* fCheckPOW= */ true, /* fCheckMerkleRoot= */ true);
    ...
    const bool valid_incl_pow = CheckBlock(block, validation_state_pow, consensus_params, seed_key, /* fCheckPOW= */ true, /* fCheckMerkleRoot= */ false);
    ...
    const bool valid_incl_merkle = CheckBlock(block, validation_state_merkle, consensus_params, seed_key, /* fCheckPOW= */ false, /* fCheckMerkleRoot= */ true);
    ...
    const bool valid_incl_none = CheckBlock(block, validation_state_none, consensus_params, seed_key, /* fCheckPOW= */ false, /* fCheckMerkleRoot= */ false);
