...
BOOST_AUTO_TEST_CASE(blockmanager_flush_block_file)
{
    ...
    //
    // Upstream expects ReadBlock to fail here, because it rechecks proof of
    // work and the block data is junk. TrueNorth's ReadBlock deliberately does
    // not recheck: under RandomX that costs ~1ms per call and every production
    // caller runs CheckBlock() afterwards, which checks PoW via
    // CheckBlockHeader(). On-disk corruption is caught by the expected_hash
    // comparison instead. See BlockManager::ReadBlock in node/blockstorage.cpp.
    // So these reads succeed, and only deserialization is exercised here.
    CBlock read_block;
    ...
    // Block 2 was not overwritten (reads successfully, and is still block 2):
    BOOST_CHECK(blockman.ReadBlock(read_block, pos2, {}));
    BOOST_CHECK_EQUAL(read_block.nVersion, 2);
}
