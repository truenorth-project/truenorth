...
// unit tests.
// Each coinbase these tests spend is worth the regtest block subsidy, which
// is not Bitcoin's 50 COIN, so amounts are taken from the coinbase itself.
...
BOOST_AUTO_TEST_CASE(package_validation_tests)
{
    ...
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    ...
                                                    /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue - 1 * COIN), /*submit=*/false);
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   ...
                                                   /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue - 2 * COIN), /*submit=*/false);
...
BOOST_AUTO_TEST_CASE(noncontextual_package_tests)
{
    ...
    // Parent and Child Package
    {
        auto mtx_parent = CreateValidMempoolTransaction(m_coinbase_txns[0], 0, 0, coinbaseKey, spk,
                                                        CAmount(m_coinbase_txns[0]->vout[0].nValue - 1 * COIN), /*submit=*/false);
        CTransactionRef tx_parent = MakeTransactionRef(mtx_parent);

        auto mtx_child = CreateValidMempoolTransaction(tx_parent, 0, 101, placeholder_key, spk2,
                                                       CAmount(m_coinbase_txns[0]->vout[0].nValue - 2 * COIN), /*submit=*/false);
        CTransactionRef tx_child = MakeTransactionRef(mtx_child);
    ...
    // 24 Parents and 1 Child
    {
        ...
        for (int i{0}; i < 24; ++i) {
            auto parent = MakeTransactionRef(CreateValidMempoolTransaction(m_coinbase_txns[i + 1],
                                             0, 0, coinbaseKey, spk, CAmount(m_coinbase_txns[0]->vout[0].nValue - 2 * COIN), false));
            package.emplace_back(parent);
            child.vin.emplace_back(COutPoint(parent->GetHash(), 0));
        }
        child.vout.emplace_back(m_coinbase_txns[0]->vout[0].nValue - 3 * COIN, spk2);
...
BOOST_AUTO_TEST_CASE(package_submission_tests)
{
    ...
    auto mtx_grandchild = CreateValidMempoolTransaction(/*input_transaction=*/tx_child, /*input_vout=*/0,
                                                       ...
                                                       /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue - 3 * COIN), /*submit=*/false);
    ...
    // In-mempool parent and child with missing parent.
    {
        auto tx_parent_1 = MakeTransactionRef(CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                                            /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                                            /*output_destination=*/parent_locking_script,
                                                                            /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue - low_fee_amt), /*submit=*/false));
        auto tx_parent_2 = MakeTransactionRef(CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[2], /*input_vout=*/0,
                                                                            /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                                            /*output_destination=*/parent_locking_script,
                                                                            /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue - 800), /*submit=*/false));
        ...
        auto tx_child_missing_parent = MakeTransactionRef(CreateValidMempoolTransaction({tx_parent_1, tx_parent_2},
                                                                                        ...
                                                                                        {{m_coinbase_txns[0]->vout[0].nValue - 1 * COIN, child_locking_script}}, /*submit=*/false));
...
BOOST_AUTO_TEST_CASE(package_single_tx)
{
    ...
    auto mtx_parent = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/parent_locking_script,
                                                    /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue) - high_fee, /*submit=*/false);
    CTransactionRef tx_parent = MakeTransactionRef(mtx_parent);
    ...
    auto mtx_child = CreateValidMempoolTransaction(/*input_transaction=*/tx_parent, /*input_vout=*/0,
                                                   /*input_height=*/101, /*input_signing_key=*/parent_key,
                                                   /*output_destination=*/child_locking_script,
                                                   /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue) - 2 * high_fee, /*submit=*/false);
    CTransactionRef tx_child = MakeTransactionRef(mtx_child);
    ...
    auto mtx_single_low_fee = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[0], /*input_vout=*/0,
                                                    /*input_height=*/0, /*input_signing_key=*/coinbaseKey,
                                                    /*output_destination=*/single_locking_script,
                                                    /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue - 1 * COIN - 1), /*submit=*/false);
...
BOOST_AUTO_TEST_CASE(package_witness_swap_tests)
{
    ...
    mtx_child1.vout[0].nValue = CAmount(m_coinbase_txns[0]->vout[0].nValue - 2 * COIN);
    ...
    auto mtx_parent1 = CreateValidMempoolTransaction(/*input_transaction=*/m_coinbase_txns[1], /*input_vout=*/0,
                                                     ...
                                                     /*output_amount=*/CAmount(m_coinbase_txns[0]->vout[0].nValue - 1 * COIN), /*submit=*/true);
    ...
    mtx_parent2_v1.vout[0].nValue = CAmount(m_coinbase_txns[0]->vout[0].nValue - 2 * COIN);
    ...
    // Spends parent1 (cb - 1 COIN), parent2_v1 (cb - 2 COIN) and parent3
    // (cb - low_fee_amt); leaves the package (parent3 + child) paying 1 COIN.
    mtx_mixed_child.vout.emplace_back(3 * m_coinbase_txns[0]->vout[0].nValue - 4 * COIN, mixed_child_spk);
