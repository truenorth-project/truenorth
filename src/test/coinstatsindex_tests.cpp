...
BOOST_FIXTURE_TEST_CASE(coinstatsindex_unclean_shutdown, TestChain100Setup)
{
    ...
    const CChainParams& params = Params();
    {
        ...
        CBlockIndex* new_block_index = nullptr;
        {
            ...
            // Test loop runs at low heights well inside the genesis epoch +
            // lag window, so the RandomX seed is kGenesisSeed for every
            // block; pass uint256::ZERO directly rather than threading it.
            BOOST_CHECK(CheckBlock(block, state, params.GetConsensus(), uint256::ZERO));
