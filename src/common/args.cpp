...
const char * const BITCOIN_CONF_FILENAME = "truenorth.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\TrueNorth
    //   new: C:\Users\Username\AppData\Local\TrueNorth
    // macOS: ~/Library/Application Support/TrueNorth
    // Unix-like: ~/.truenorth
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "TrueNorth";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "TrueNorth";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/TrueNorth";
#else
    // Unix-like
    return pathRet / ".truenorth";
#endif
#endif
}
